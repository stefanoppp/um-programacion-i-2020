...
class Contador(Texto):

    def generar(self, t):

        for palabra in t.split(" "):
            self.d[palabra] = t.count(palabra)           #Coloca todas los valores junto a su elemento

        self.orden = sorted(self.d.items(), key=operator.itemgetter(1),
                            reverse=True)
        self.orden_alfa = sorted(self.d.items(), key=operator.itemgetter(0))  #(1) orden por numeros,(0) por letras    

        return(self.orden, self.orden_alfa)

    def pantalla(self, orden, orden_alfa):
